package com.openle.our.core.compiler;
...
/**
 * Utility class to generate Java source code. Each instance of
 * {@link JavaCodeGenerator} defines a scope. This is used to keep track of
 * names declared in each scope and allows for variable name shadowing. A new
 * scope is created on blocks and for-loops following java scoping rules. Scopes
 * also collect code emitted in them, once the scope is left, all code emitted
 * is applied to its parent scope. The {@link JavaCodeGenerator} also define a
 * parent-child relationship, where each child defines a new scope based on the
 * current scope of it's parent. This is used to support, for example, defining
 * new methods/fields at a class level while generating code for a method.
 * Another way to see it is that considering the scopes defined by a java
 * compilation unit, each JavaCodeGenerator can generate code on a specific
 * scope subtree. So, for example, one can have a code generator for the class,
 * and one for each method.
 */
...
public class JavaCodeGenerator {
    ...
    /*
     * Creates a new JavaCodeGenerator that generates in a sub-scope of the
     * specified generator
     */
    ...
    /*
     * @return the name with a unique integer appended. The number is unique
    ...
    /*
     * Declares an initialized local variable at the beginning of the current
     * scope. Useful to declare temporary variables on complex expressions.
     */
    ...
    /*
     * Prints the specified code at the beginning of the current scope. Useful
     * to initialize temporary variables on complex expressions.
     */
    ...
    /*
     * @return the mangled name of the variable if it's shadowing another on an
     * outer scope
     */
