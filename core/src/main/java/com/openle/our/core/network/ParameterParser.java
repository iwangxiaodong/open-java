...
public class ParameterParser {
	 ...
	 * 
	 * @param b
	 *            <tt>true</tt> if parameter names are to be converted to lower
	 *            case when name/value pairs are parsed. <tt>false</tt>
	 *            otherwise.
	 */
		...
		while (hasChar()) {
			...
			if ((paramName != null) && (paramName.length() > 0)) {
				if (this.lowerCaseNames) {
					paramName = paramName.toLowerCase(Locale.getDefault());
				}
